...
const Page = async() => {
  ...
  <div className="container px-5 py-24 mx-auto">
    ...
    {Object.keys(products).length===0 && <p>Sorry,All the Tracksuits currently are out of stock. New Stock Will be available soon. Stay Tuned</p>}  
    {Object.keys(products).map((item)=>{ 
      ...
      <Link passHref={true} href={`/product/${products[item].slug}`} className="block relative rounded overflow-hidden ">
        ...
        <div className="mt-4 text-center ">
          <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">Track Suit</h3>
...
let products= await Product.find({category:'TrackSuits'})
let TrackSuits={}
for(let item of products){
        if(item.title in TrackSuits){
               if(!TrackSuits[item.title].color.includes(item.color) && item.availableQty>0){
                TrackSuits[item.title].color.push(item.color)
               }
               if(!TrackSuits[item.title].size.includes(item.size) && item.availableQty>0){
                TrackSuits[item.title].size.push(item.size)
               }
        }
        else{
              TrackSuits[item.title]=JSON.parse(JSON.stringify(item))  
              if(item.availableQty>0){
                TrackSuits[item.title].color=[item.color]
                TrackSuits[item.title].size=[item.size]
              }
        }
}
return JSON.parse(JSON.stringify(TrackSuits))
}
