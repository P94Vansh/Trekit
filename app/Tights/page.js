...
const Page = async() => {
  ...
  <div className="container px-5 py-24 mx-auto">
    ...
    {Object.keys(products).length===0 && <p>Sorry,All the Tights currently are out of stock. New Stock Will be available soon. Stay Tuned</p>}  
    {Object.keys(products).map((item)=>{ 
      ...
      <Link passHref={true} href={`/product/${products[item].slug}`} className="block relative rounded overflow-hidden ">
        ...
        <div className="mt-4 text-center ">
          <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">Tights</h3>
...
let products= await Product.find({category:'Tights'})
let Tights={}
for(let item of products){
        if(item.title in Tights){
               if(!Tights[item.title].color.includes(item.color) && item.availableQty>0){
                Tights[item.title].color.push(item.color)
               }
               if(!Tights[item.title].size.includes(item.size) && item.availableQty>0){
                Tights[item.title].size.push(item.size)
               }
        }
        else{
              Tights[item.title]=JSON.parse(JSON.stringify(item))  
              if(item.availableQty>0){
                Tights[item.title].color=[item.color]
                Tights[item.title].size=[item.size]
              }
        }
}
return JSON.parse(JSON.stringify(Tights))
}
